...
- 鼠标悬停在点事件命中区域时使用手形光标，提示可拖动。

区间事件实例：

- 渲染为从起点时间延伸到终点时间的水平段。
- 使用事件颜色，并带有轻微边框。
- 区间主体使用半透明填充，让重叠区间能够被看出。
- 在单行轨道内保留最小可见高度。
- 渲染可见的左右尺寸调整手柄。
- 每个边缘手柄的视觉宽度为 `3` px，水平命中区域至少为 `10` px。
- 鼠标悬停在区间主体中间时使用手形光标，提示可整体拖动。
- 鼠标悬停在区间左右边缘时继续使用水平调整光标，提示可调整边界。
...
- 已完成数量加一。
- 如果同一个事件轨道内已经存在完全相同时间的点实例，则不创建新实例，防止用户连续点击按钮但没有移动播放头时产生重复标记。
...
- 清除待完成状态，并让已完成数量加一。
- 如果同一个事件轨道内已经存在完全相同起止时间的区间实例，则不创建新实例，只清除待完成状态，防止重复标记。
...
- 右键点击事件轨道时间轴区域的空白处时打开轨道菜单。
- 轨道菜单包含 `删除所有“事件名”`。
- 选择轨道级删除会移除该事件轨道的所有已完成点/区间实例，并清除等待设置终点的区间起点。
...
- 点事件和区间事件不会在相同时间重复创建完全相同的实例。
- 轨道级删除会清空实例计数和实例列表。
- 轨道级删除会清除等待设置终点的区间起点状态。
- 点事件左侧信息行使用圆点类型指示符，区间事件使用竖条类型指示符。
- 点事件和区间主体使用手形光标，区间边缘保持水平调整光标。
- 区间主体具备半透明样式。
- 轨道空白区域右键菜单包含 `删除所有“事件名”`。
